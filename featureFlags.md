...
| `enable_timesheet1_importer` | Disabled | Adds a modal on the Time Entries page to import legacy Timesheet 1.0 spreadsheets from Drive with hour-type mapping, contract validation, and duplicate checks. |
...
| `custom_theme` | Disabled | Adds a Custom theme option with an eight-colour palette (background, surface, text, muted, primary, success, warning, danger). Selecting Custom enables the configure button to open a live preview and Tempus checks contrast before saving while generating complementary shades across the interface. |
...
| `enable_assessments_mode` | Disabled | Unlocks the Assessments workflow: a month-centric screen for recording per-assessment work with auto-linked billable entries, per-contract assessment type configuration, an auto-created "Assessment work" hour type, and assessment-driven monthly invoice generation. Designed for users who bill per assessment rather than per hour. |
| `enable_expense_report` | Disabled | Adds an Expense Report section to the BAS Reporting screen showing the selected financial year's company expenses grouped by category, with a CSV download suitable for sharing with an accountant. |
