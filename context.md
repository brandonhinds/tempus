...
6. Customization depth: How granular should the customization be? (UI themes, field visibility, workflow steps, etc.)
    ...
    The Settings screen now mirrors this philosophy: every control is rendered inside a collapsible “Core” section or a matching feature-flag section. Sections only appear when their governing flag is enabled, so new settings must be registered with the same snake_case identifier as the flag that unlocks them. This keeps the form tidy while still surfacing extra options as users opt into experiments.
...
- Gross income across the dashboard and annual views is derived from the contract total package (hourly rate × hours): we subtract any company expenses first, then divide by `(1 + super rate)` so the gross figure excludes employer super while keeping super/tax/net numbers consistent even when company tracking is disabled.
...
- When the `contract_rate_preview` flag is enabled a dedicated Rate Preview page appears, letting you pick a contract, model either percentage or absolute hourly rate changes, and review server-calculated lifetime and month-by-month variances across gross income, superannuation, tax, and net earnings.
- Rate Preview treats contract hourly rates as total package values and applies the same gross-derivation logic before estimating super, tax, and net so the preview matches the dashboard calculations.
...
- When the `recurring_time_entries` flag is enabled the Time page exposes a “Manage Reoccurring Entries” modal where users can define weekly or monthly cadences (hours per occurrence, hour type, contract, start/end dates). The backend sync job only generates basic entries that fall inside active contract windows (capping support to contracts <= 3 years), surfaces warnings when a contract is ignored, backfills non-income hour types, and limits income-generating schedules to future dates.
- When the `bulk_time_entries` flag is enabled the Time page exposes a “Bulk Entries” modal that lets users specify hours per day, contract/hour type, start/end dates, and whether to include weekends or skip public holidays. Saving a bulk range automatically creates (or removes) entries for every day in the window, and editing the range keeps the generated entries in sync without manual clean-up.
...
- The Deductions page lets users configure personal salary sacrifice items, company deductions (when the `enable_company_tracking_features` flag is on), and extra super contributions. Deductions may be one-off or recurring, support GST-inclusive inputs for company expenses, and feed directly into the monthly income breakdown (including "Superannuation lost due to deductions" and extra super totals). When the `enable_deduction_categories` flag is enabled, deductions can be tagged with colour-coded categories, managed from a dedicated modal, and the list groups cards under collapsible category headings. A notes field is available now; file attachments will be added in a future iteration.
- Annual views include a category breakdown table (behind `enable_deduction_categories`) that summarises monthly expenses per category, allows expanding a column to reveal per-deduction amounts, and offers chip-style filters to focus on specific categories.
...
- BAS reporting lives on its own page once company tracking is enabled. It defaults to monthly rows across the financial year (Jul–Jun) and can switch to quarterly totals via the `enable_company_quarterly_bas` flag. Each period shows invoice totals (with GST), company income, business expenses, and the employee payroll figures (gross, super, deductions, tax, net). When the invoices feature flag is on, a companion table lists every invoice issued in the selected financial year with line counts, hours, and GST-inclusive totals to support BAS reconciliation. The page uses a widened layout on desktop (up to 1600px) so the BAS table can display all columns without feeling cramped. The supporting `ts-container--fluid` helper is reserved for rare extra-wide layouts; keep other pages on the standard 1280px container.
- When `enable_invoices` is on an Invoices page appears with month filters, optimistic header and line item editing, and a defaults drawer for frequently-used line definitions. Line items can optionally specify hours and hour types—when hours are present we immediately create/update a basic timesheet entry and surface a “modified” warning if the calendar entry later drifts. The Settings page now exposes invoice controls (template reference, output folder path, and placeholder limit) which must be populated before the “Publish invoice” workflow is enabled.
- Invoice documents are generated by cloning the user’s Google Docs template, then replacing `{{invoiceNumber}}`, `{{invoiceDate}}`, `{{invoiceTotal}}`, `{{invoiceTotalHours}}`, and per-line placeholders (`{{lineDate#}}`, `{{lineDescription#}}`, `{{lineHours#}}`, `{{lineHourType#}}`, `{{lineRate#}}`, `{{lineAmount#}}`, `{{lineContract#}}`) up to the configured line limit. `{{lineRate#}}` is derived at render time from the saved amount and hours rather than a dedicated sheet column. The resolved template and output folder IDs are cached on the invoice record so re-generation reuses the same sources.
- A new feature flag, `expected_monthly_hours`, adds a calendar badge that surfaces the average daily hours recorded for the active month. The breakdown leans on the reworked metrics (average daily hours now divides income-generating hours by the number of days with income entries, projected hours this month factors in the selected trend method, and the capacity rows highlight totals “based on available business days”), plus the “Monthly insights trend calculation” setting defaults to the Standard contract hours option and a companion “Projections variance badge value” setting lets users choose which metric powers the badge (monthly capacity, contract pace, or either projected-vs-target delta). When expanded it breaks the projection down per contract, showing the days included, expected hours remaining, and whether the user is trending ahead or behind their contracted target based on income-contributing hour types only.
