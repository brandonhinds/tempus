...
| `use_for_rate_calculation` | boolean/string | `TRUE`/`FALSE` indicating if this hour type's hours should be used for hourly rate calculations in annual views and income breakdown. Multiple hour types can be marked to allow tracking of related work categories. | `FALSE` |
...
### Built-in hour types
- The "Work" hour type (`slug: "work"`) is automatically created and cannot be deleted. It contributes to income, requires a contract, is the default when the hour_types feature is disabled, and is used for rate calculation if no other hour types have that flag set.
