...
// A time-of-day setting (e.g. minimum_end_time) written as "17:00" gets auto-coerced by Sheets into a
// time-typed cell, which reads back as a Date on the 1899-12-30 epoch and reaches the client as a full
// datetime ("1899-12-30T07:00:00.000Z") that an <input type="time"> rejects. Convert those back to
// "HH:mm" in the script timezone (where the value was authored). Anything else is left untouched.
function normalizeSettingsValues(obj) {
  ...
  Object.keys(obj).forEach(function(k) {
    ...
    if (d && d.getUTCFullYear() < 1900) {
      obj[k] = Utilities.formatDate(d, tz, 'HH:mm');
    }
  });
  return obj;
}
...
function api_getSettings() {
  ...
  if (cached && typeof cached === 'object' && Object.keys(cached).length > 0) {
    ...
    return normalizeSettingsValues(cached);
  }
  ...
  cacheSet(key, out);
  return normalizeSettingsValues(out);
}
