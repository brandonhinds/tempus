...
var RECURRING_ENTRY_HEADERS = [
  ...
  'created_at',
  'updated_at',
  // Appended LAST (Phase 6, sessions on schedules) so existing rows stay column-aligned; the header
  // reset in ensureRecurringEntriesSchema picks it up on first access. '' = whole-hours schedule.
  'sessions_json'
];

// Sessions on a schedule (Phase 6): closed [{in,out}] spans (24h HH:MM). Reuses the entries
// normalizePunches; open spans are dropped — a schedule can only stamp complete sessions.
function parseScheduleSessions(value) {
  if (!value) return [];
  return normalizePunches(value).filter(function(p) { return p['in'] && p.out; });
}
...
function normalizeRecurringEntryRow(entry) {
  ...
  var durationMinutes = sessions.length ? punchesTotalMinutes(sessions) : normalizeDurationMinutes(entry.duration_minutes);
...
function normalizeRecurringPayload(payload) {
  ...
  // Sessions schedule: duration is DERIVED from the spans; a whole-hours schedule keeps the typed value.
  ...
  if (!durationMinutes || durationMinutes <= 0) {
    throw new Error(sessions.length ? 'Sessions must have an end after their start.' : 'Duration must be greater than zero minutes.');
  }
...
function processRecurringEntry(entry, context) {
  ...
  if (!endBound || endBound < startBound) {
    // The schedule's date range and the contract's active window don't overlap (e.g. an FY contract that
    // ends before the schedule's dates). Name the real cause rather than the misleading "end precedes start".
    summary.warning = 'Contract "' + contract.name + '" is not active for this schedule\'s date range.';
    if (entry.warning_message !== summary.warning) {
      entry.warning_message = summary.warning;
      entryChanged = true;
    }
...
function createRecurringEntryInstance(entry, dateIso, context) {
  try {
    // Sessions schedule → a real advanced entry carrying the spans; whole-hours → basic as before.
    var sessions = parseScheduleSessions(entry.sessions_json);
